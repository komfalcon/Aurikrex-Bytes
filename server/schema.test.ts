...
describe("Aurikrex Bytes schema", () => {
  it("defines the required publishing and auth tables", () => {
    ...
    expect(Object.keys(posts)).toEqual(expect.arrayContaining(["id", "imageUrl", "headline", "body", "status", "scheduledTime", "publishedTime", "createdBy", "updatedAt"]));
    expect(Object.keys(adminUsers)).toEqual(expect.arrayContaining(["id", "email", "passwordHash", "role", "rememberDeviceToken", "createdAt"]));
