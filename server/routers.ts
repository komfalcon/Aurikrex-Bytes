...
export const appRouter = router({
  ...
  auth: router({
    ...
    logout: publicProcedure.mutation(({ ctx }) => { const cookieOptions = getSessionCookieOptions(ctx.req); for (const name of [COOKIE_NAME, "aurikrex_admin_session", "aurikrex_admin_device", "aurikrex_reader_session", GOOGLE_STATE_COOKIE, GOOGLE_NONCE_COOKIE]) ctx.res.clearCookie(name, { ...cookieOptions, maxAge: -1 }); return { success: true } as const; }),
  }),
  admin: router({
    ...
    post: publicProcedure.input(z.object({ id: z.number().int().positive() })).query(async ({ input, ctx }) => { await requireAdmin(ctx); const post = await getPostById(input.id); if (!post) throw genericNotFound(); return post; }),
    createPost: publicProcedure.input(z.object({ headline: z.string().min(1).max(120), body: z.string().min(1).max(800), imageUrl: z.string().url().optional() })).mutation(async ({ input, ctx }) => { const admin = await requireAdmin(ctx); assertPermission(admin.role, "post:create"); const db = await getDb(); if (!db) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Database is not configured" }); const result = await db.insert(posts).values({ ...input, status: "draft", createdBy: admin.id, updatedAt: new Date() }); return { success: true, id: Number(result.lastInsertRowid) }; }),
    ...
    approvePost: publicProcedure.input(z.object({ id: z.number().int().positive(), scheduledTime: z.coerce.date().optional() })).mutation(async ({ input, ctx }) => { const admin = await requireAdmin(ctx); assertPermission(admin.role, "post:review"); const db = await getDb(); const post = await getPostById(input.id); if (!db || !post) throw genericNotFound(); if (input.scheduledTime && input.scheduledTime <= new Date()) throw new TRPCError({ code: "BAD_REQUEST", message: "scheduledTime must be in the future" }); const next = input.scheduledTime ? "scheduled" : "published"; assertPostTransition(admin.role, post.status, next); await db.update(posts).set({ status: next, scheduledTime: input.scheduledTime ?? null, publishedTime: input.scheduledTime ? null : new Date(), rejectionNote: null, updatedAt: new Date() }).where(eq(posts.id, input.id)); return { success: true, status: next }; }),
  ...
  reader: router({
    ...
    resetPassword: publicProcedure.input(z.object({ token: z.string().min(10), password: z.string().min(8) })).mutation(async ({ input }) => { if (!isValidPassword(input.password)) throw new TRPCError({ code: "BAD_REQUEST", message: "Password must be at least 8 characters and include a number and symbol" }); const reader = await getReaderByResetToken(input.token); if (!reader) throw new TRPCError({ code: "BAD_REQUEST", message: "Invalid or expired reset token" }); const db = await getDb(); if (!db) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Database is not configured" }); await db.update(readers).set({ passwordHash: await hashPassword(input.password), resetToken: null, resetTokenExpires: null }).where(eq(readers.id, reader.id)); return { success: true }; }),
